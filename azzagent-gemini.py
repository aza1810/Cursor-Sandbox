...
1. Put your Gemini API key below.
2. Run this script from the root of the project you want it to work on:
       python3.9 /home/tc/azzagent.py
"""
...
def shell(args):
    ...
    try:
        ...
        return (
            "exit=%d\n%s" % (result.returncode, result.stdout or "")
        )[:MAX_OUTPUT_CHARS]
...
def call_gemini(prompt):
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "AzzAgent-Gemini/0.3",
        },
        method="POST",
    )
    ...
    try:
        ...
        raise RuntimeError(
            "Gemini API HTTP %s: %s" % (e.code, body[:2000])
        )
...
def run_turn(user_text):
    ...
    for _ in range(MAX_TOOL_STEPS):
        ...
        prompt = build_prompt(
            "Continue the task. The latest tool result is:\n" + result
        )
...
def main():
    ...
    if (
        not GEMINI_API_KEY
        or GEMINI_API_KEY == "PUT_YOUR_GEMINI_API_KEY_HERE"
    ):
        print("\nERROR: Put your Gemini API key into GEMINI_API_KEY near the top of this file.")
        return 1
